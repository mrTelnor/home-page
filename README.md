...
- 💾 **Бэкапы** — ежедневный `pg_dump` БД + архив фото рецептов с ротацией 14 дней на Яндекс.Диск через WebDAV (загрузка с ретраями); при провале дампа, архивации или загрузки админы получают алерт в Telegram
...
Ежедневно в **03:00 GMT+3** тот же cron выполняет `pg_dump -Fc` БД и tar.gz-архив фото рецептов, заливает оба файла на Яндекс.Диск через WebDAV (с ретраями) и удаляет бэкапы старше 14 дней. Остальные cron-задачи вызывают backend/бот с `curl --fail` и ретраями; провал любого шага — алерт админам в Telegram.
