...
- 📅 **Интеграция с Google Calendar** — бот читает несколько семейных календарей через service account и шлёт админам почасовые напоминания о событиях, поддерживает встроенные напоминания из календаря, утренний дайджест в 8:00 (вместе с меню) и команду `/schedule` для запроса расписания на сегодня и завтра по требованию
- 🎓 **Электронный дневник** — интеграция с `app.eschool.center`: автоматические уведомления админам-Волковым об оценках за день (18:00 МСК) и о ДЗ на следующий школьный день (15:00 МСК + push-апдейты до 22:30); сам ученик получает уведомления о своих ДЗ
...
## Расписание eschool (GMT+3)

| Время | Действие |
|---|---|
| 15:00 | Дайджест ДЗ на следующий школьный день |
| 15:30, 16:00, ..., 22:30 | Push-апдейты по новым ДЗ (каждые 30 мин) |
| 18:00 | Дайджест оценок за сегодня |

Логика «следующий школьный день»: пн–чт → завтра, пт → понедельник, сб/вс → понедельник (с пометкой «Напоминаю»).
