...
- 📖 **База рецептов** — добавление, редактирование, удаление рецептов с ингредиентами. Пересчёт ингредиентов под произвольное количество порций. Сортировка списка по алфавиту, дате добавления или дате изменения (запоминается в браузере)
...
- 🤖 **Telegram-бот** — просмотр меню, голосование, предложение рецептов, уведомления о событиях (новое меню, открытие голосования, результаты). Настройка уведомлений per-user (/mute, /unmute)
...
- 🌐 **Веб-интерфейс** — адаптивный UI с тёмной темой, валидация форм с подсветкой ошибок
...
| Telegram-бот | Python 3.12, Aiogram 3, httpx |
| Автоматизация | Cron-контейнер (Alpine + curl) для ежедневного цикла голосования + уведомлений |
| Инфраструктура | Docker, Docker Compose, Traefik v3 (Let's Encrypt), Ansible, Cloudflare DNS |
...
| `bot.telnor.ru` | Telegram-бот (webhook) | публично (для Telegram) |
