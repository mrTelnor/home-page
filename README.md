...
- 📖 **База рецептов** — добавление, редактирование, удаление рецептов с ингредиентами. Пересчёт ингредиентов под произвольное количество порций. Сортировка списка по алфавиту, дате добавления или дате изменения (запоминается в браузере). У рецепта может быть **фото** (задаётся URL в форме; backend скачивает и хостит его сам, раздаёт через `/api/recipe-images/...`); без фото показывается SVG-иконка (15 типов × 10 палитр), выбираемая вручную или по хешу названия. Массовый импорт рецептов с фото, количествами и пошаговым рецептом — скриптами в `tools/` (см. ниже)
...
- 🤖 **Telegram-бот** — просмотр меню, голосование, предложение рецептов, уведомления о событиях (новое меню, открытие голосования, результаты). Настройка уведомлений per-user: `/notifications` — отдельно ужины и семейный календарь (inline-кнопки), `/mute` и `/unmute` — всё сразу; то же — чекбоксами в профиле на сайте. Поиск рецептов по названию через `/suggest`
- 📅 **Интеграция с Google Calendar** — бот читает несколько семейных календарей через service account и шлёт админам почасовые напоминания о событиях (каждый админ может отключить их отдельно от ужинов), поддерживает встроенные напоминания из календаря и календарные настройки `useDefault` (через env `CALENDAR_DEFAULT_REMINDERS_MIN`, по умолчанию `30`), утренний дайджест в 8:00 (вместе с меню) и команду `/schedule` для запроса расписания на сегодня и завтра по требованию
...
Работает в режиме **polling** (Telegram API не блокирует исходящий трафик из bot-контейнера благодаря WireGuard-туннелю — в туннель идут только подсети Telegram, остальной трафик напрямую). При вставке нового конфига от VPN-провайдера переносить только ключи и Endpoint: строка `DNS=` ломает резолвинг всей ВМ (инцидент 2026-06-10).

Помимо polling, бот поднимает aiohttp-сервер на порту `8080`:
- `GET /healthz` — проверка связности с Telegram API (`get_me`): 200 при живом транспорте, 503 если Telegram недоступен. Рекомендуется HetrixTools-монитор на `https://bot.telnor.ru/healthz` — обычный uptime-мониторинг не ловит сценарий «HTTP жив, а polling мёртв»
- `POST /alert` (X-Cron-Secret) — рассылка произвольного текста админам; используется cron-контейнером для алертов о провале бэкапа
