...
Query-параметры:
- `digest=true` — отправить утренний дайджест (события на сегодня и завтра, разделённые по календарям)
- `force=true` — игнорировать дедупликацию (для дайджеста — отправить даже если уже был сегодня)

Без параметров — проверка событий в окне ~24 ч и рассылка напоминаний за 1 час до начала + встроенных reminders из событий.
...
Дедуп: бот хранит отправленные напоминания в `/data/sent_reminders.json` (Docker volume `bot_data`), записи старше 7 дней удаляются автоматически.
