...
События:
- `menu_created` — меню дня создано (08:00). Не-админам — короткое уведомление; админы получают это в составе утреннего дайджеста.
...
Query-параметры:
- `digest=true` — отправить утренний дайджест (события на сегодня и завтра, разделённые по календарям, + текущее меню)
- `force=true` — игнорировать дедупликацию (для дайджеста — отправить даже если уже был сегодня)

Без параметров — проверка событий в окне ~24 ч и рассылка напоминаний за 1 час до начала + встроенных reminders из событий + catch-up для voting-уведомлений (если разовый `/notify` пропал из-за рестарта/сбоя — бот сам дошлёт `voting_opened`/`voting_closed` с дедупом по `menu_id`).

Ручное тестирование:
```bash
# дайджест на сегодня + завтра (повторно — с force)
docker exec cron sh -c 'curl -s -X POST "http://bot:8080/check-calendar?digest=true&force=true" \
  -H "X-Cron-Secret: $CRON_SECRET"'

# почасовая проверка вне расписания
docker exec cron sh -c 'curl -s -X POST http://bot:8080/check-calendar \
  -H "X-Cron-Secret: $CRON_SECRET"'
```

Дедуп: бот хранит отправленные напоминания в `/data/sent_reminders.json` (Docker volume `bot_data`), записи старше 7 дней удаляются автоматически. Ключи дедупа:
- `digest:YYYY-MM-DD` — утренний дайджест
- `60min:<calendar_id>:<event_id>:<start>` — часовое напоминание
...
> Интеграция с eschool была удалена. Краткая история и причины отказа — в [docs/eschool_decommission.md](eschool_decommission.md).
