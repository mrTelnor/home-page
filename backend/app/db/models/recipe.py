...
class Recipe(Base, UUIDMixin, TimestampMixin):
    ...
    # created_at — сортировка списка рецептов по умолчанию
    __table_args__ = (
    ...
    author_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("auth.users.id"), index=True)
...
class Ingredient(Base, UUIDMixin):
    ...
    recipe_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("dinner.recipes.id", ondelete="CASCADE"), index=True)
